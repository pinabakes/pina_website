class PinaBakesApp {
  ...
  setupEventListeners() {
    ...
    if (this.elements.checkoutForm) this.elements.checkoutForm.addEventListener('submit', this.checkout.handleFormSubmit.bind(this));
  ...
  gallery = {
    ...
    renderThumbnails: (images, productName) => {
      ...
      this.elements.productThumbnails.innerHTML = images.map((image, index) => `
        <img src="${image}" alt="${productName} - Thumbnail ${index + 1}" class="product-thumbnail ${index === 0 ? 'active' : ''}" onclick="App.gallery.selectImage(${index})" loading="lazy">
      `).join('');
    },
    selectImage: (index) => {
  ...
  cart = {
    ...
    render: () => {
      ...
      if (this.elements.shippingNote) this.elements.shippingNote.textContent = `Shipping ₹${this.config.shippingCharge} applies below ₹${this.config.freeShippingThreshold}. Free shipping on orders ₹${this.config.freeShippingThreshold}+`;
